    ...
    for (let monthOffset = -2; monthOffset <= 0; monthOffset++) {
      ...
      for (let day = 1; day <= monthEnd.getDate(); day++) {
        ...
        if (dayChecks.length > 0) {
          const failures = dayChecks.filter(check => check.status === 'down').length;
  ...
  return (
    <div className="mt-8">
      <div className="overflow-hidden rounded-lg bg-white shadow">
        <div className="p-6">
          <div className="mb-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">3-Month Status Calendar</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {calendarData.map((month, monthIndex) => {
              const monthDate = new Date(today.getFullYear(), today.getMonth() - 2 + monthIndex, 1);
              return (
